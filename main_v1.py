...
# === CLOUD PATH FIX ===
# This tells Python: "Look in the folder where this script is running"
...
def read_pdf_directly(file_input):
    ...
    Accepts: String (filename) or List of Strings (filenames)
    """
    ...
    for filename in file_list:
        ...
        try:
            # Handle Text Files (like the Glossary)
...
with st.sidebar:
    ...
    # === CLOUD-PROOF ENGINE SWITCHER ===
    # Check if we are on Streamlit Cloud (Streamlit sets 'HOSTNAME' usually, or we can just default to Cloud)
    # Simple Trick: If the secret for Google API exists, assume Cloud preference.
    
    engine_options = ["☁️ Gemini (Google)"]
    
    # Only show Local option if NOT running on Cloud (You can toggle this manually)
    SHOW_LOCAL = False  # <--- SET TO FALSE BEFORE UPLOADING TO GITHUB
